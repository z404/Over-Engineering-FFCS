    ...
    fetch("/scorechange/", {
        ...
        .then((json) => {
            const ele = document.getElementById(
                "displayPriority" +
                (Number(document.getElementById("timetable-index").innerText) - 1)
            );
            if (newScore == 1) {
                ele.innerHTML =
    ...
    const updateTimetable = (json) => {
        ...
        const createInfoTable = (info_dict) => {
            const createRow = (rowData, idx) => {
                ...
                [idxElement, name, courseCode, erpid, slots, subj].forEach((el) =>
            ...
            ["##", "Employee Name", "Course Code", "ERP", "Slot", "Subject"].forEach(
                (el) => header.appendChild(createDataElement("th", el))
            );
        ...
        document.getElementById("nickname-box").value =
            nickname.childNodes[0].nodeValue;
        ...
        document
            .getElementById("nickname-" + String(json["index"]))
            .classList.add("current");
        var height = $("#boxifycontent").height();
        var otherheight = $("#index-button").height();
        document.getElementById("timetablelist").style.maxHeight =
            height - otherheight - 12 + "px";
    };
    try {
        ...
        fetch("/rendertimetable/", {
            ...
            .then((json) => {
                updateTimetable(json);
                let tempHolder = JSON.parse(
                    localStorage.getItem("allstoredtimetables")
                );
                if (tempHolder == null) {
                    tempHolder = {};
                }
...
const nicknameChange = () => {
    ...
    fetch("/nicknamechange/", {
        ...
        .then((json) => {
            ...
            ele.innerText =
                "#" +
                document.getElementById("timetable-index").innerText +
                ": " +
                newnick;
            let ele2 = document.getElementById("nicknameontop");
            ele2.innerText = newnick;
            const tempHolder = JSON.parse(
                localStorage.getItem("allstoredtimetables")
            );
            try {
